...
re_FuncDefStart = re.compile("\s*(?P<retType>%s)?\s*(?P<name>\w*)\s*\((?P<params>%s)?(?:(?P<endComma>,$)|(?P<openBrace>\)\s*\{\s*$))" % (RetTypes, FuncParams))
re_FuncDefArgLine = re.compile("\s*(?P<params>%s)(?:(?P<endComma>,)|(?P<openBrace>\)\s*\{))" % (FuncParams))
re_FuncDeclStart = re.compile("\s*(?P<retType>%s)?\s*(?P<name>\w*)\s*\((?P<params>%s)?(?:(?P<endComma>,$)|(?P<openBrace>\)\s*;\s*$))" % (RetTypes, FuncParams))
