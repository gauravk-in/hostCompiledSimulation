...
memAccessCycles += simICache(0xaf0, 16);
pipelineCycles += 12 - (enterBlock(0x2da, 0x2dd) ? 7 : 0);
  a = 0;
  i = 0;
  r = 0;
...
pipelineCycles += 13 - (enterBlock(0x2de, 0x2e9) ? 7 : 0);
  D_2427 = r << 2;
  r = (x >> 30) + D_2427;
  x = x << 2;
  a = a << 1;
...
memAccessCycles += simICache(0xb00, 48);
  i = i + 1;
  if (i != 32)
    goto usqrtbb_3;
  else
...
memAccessCycles += simICache(0xb30, 12);
pipelineCycles += 9 - (enterBlock(0x2ea, 0x2ec) ? 7 : 0);
